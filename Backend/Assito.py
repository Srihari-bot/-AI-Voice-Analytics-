...
# Try to import rag_pipeline, but make it optional for Assisto API
try:
...
def transcribe_audio_file_attempt(audio_file: UploadFile, attempt: int = 1) -> str:
    """
    Single attempt to transcribe audio file using Assisto API.
    """
    ...
    logger.info(f"Processing audio file: {original_filename} (attempt {attempt})")
        ...
        with open(tmp_file_path, "rb") as file:
            ...
            if response.status_code == 200:
                try:
                    ...
                    # Check for Assisto API response structure
                    if isinstance(result, dict):
                        ...
                        for key in possible_keys:
                            if key in result:
                                ...
                                if isinstance(data, str) and data.strip():
                                    ...
                                    logger.info(f"Found transcription in key '{key}': {transcription[:200]}...")
                                    break
                                    ...
                                    for item in data:
                                        if isinstance(item, dict):
                                            # Look for text/transcript in each item
                                            for text_key in ['text', 'transcript', 'content']:
                                                if text_key in item and isinstance(item[text_key], str):
                                                    segments.append(item[text_key].strip())
                                                    break
                                    ...
                                    if segments:
                                        ...
                                        logger.info(f"Combined {len(segments)} transcript segments from list")
                                        break
                    
                    # If result is a string directly
                    if transcription is None and isinstance(result, str):
...
@retry_with_backoff(max_retries=4, base_delay=3, max_delay=60)
def transcribe_audio_file(audio_file: UploadFile) -> str:
    """
    Transcribe audio file using Assisto API with enhanced error handling and retries for token expiration.
    """
    try:
        return transcribe_audio_file_attempt(audio_file, attempt=1)
    except APIError as api_err:
        if api_err.status_code == 401:
            logger.warning("Assisto API token expired, will retry with fresh environment reload")
            # Force reload environment variables to get a fresh API key
            reload_env_variables()
            # Retry the request
            return transcribe_audio_file_attempt(audio_file, attempt=2)
        else:
...
async def test_all_services_connectivity():
    ...
    # Test Assisto AI connectivity
    try:
        assisto_url = 'https://dev.assisto.tech/workflow_apis/process_file'
        api_key = os.getenv("ASSISTO_API_KEY")
        headers = {'Authorization': f'Bearer {api_key}'} if api_key else {}
        
        assisto_result = await test_service_connectivity(assisto_url, headers, timeout=15)
        results["assisto_ai"] = {
            "url": assisto_url,
            "status": "connected",
            "details": assisto_result
        }
...
async def refresh_environment_variables():
    ...
    try:
        ...
        assisto_api_key = get_env_var("ASSISTO_API_KEY")
        ...
        return {
            ...
            "api_key_previews": {
                ...
                "assisto": f"{assisto_api_key[:4]}...{assisto_api_key[-4:]}" if assisto_api_key and len(assisto_api_key) >= 8 else "not_configured"
            }
        }
    except Exception as e:
...
@app.get("/debug-assisto-api")
async def debug_assisto_api():
    """Debug endpoint to test Assisto API response structure."""
    try:
        ...
        debug_info = {
            "api_url": url,
            "headers_sent": {"auth_header_present": bool(headers.get("Authorization"))},
...
@app.get("/test-assisto-credentials")
async def test_assisto_credentials():
    """Test if Assisto API credentials are valid."""
    try:
        api_key = get_env_var("ASSISTO_API_KEY", reload=True)
        
        if not api_key:
            return {
                "valid": False,
                "message": "Assisto API key not configured",
                "instruction": "Please set ASSISTO_API_KEY in your .env file"
            }
        
        if api_key == "your_actual_assisto_api_key_here":
            return {
                "valid": False,
                "message": "Assisto API key still using placeholder value",
                "instruction": "Please update ASSISTO_API_KEY in your .env file with your actual Assisto API key"
            }
        ...
        if len(api_key) < 10:
            return {
                "valid": False,
                "message": "Assisto API key appears to be too short",
                "instruction": "Please check your ASSISTO_API_KEY in the .env file"
            }
        ...
        try:
            ...
            return {
                "valid": True,
                "message": "Assisto API key is configured and API is reachable",
                "api_key_format": f"{api_key[:4]}...{api_key[-4:]}" if len(api_key) >= 8 else "short_key",
            ...
            return {
                "valid": True,
                "message": "Assisto API key is configured but connectivity test failed",
                "api_key_format": f"{api_key[:4]}...{api_key[-4:]}" if len(api_key) >= 8 else "short_key",
        ...
        logger.error(f"Assisto credential test error: {str(e)}")
        return {
            "valid": False,
            "message": f"Error testing Assisto credentials: {str(e)}",
            "instruction": "Check your .env file configuration"
        }
...
async def generate_resolution_endpoint(data: dict):
    ...
    try:
        ...
        # RAG Enhancement: Retrieve context (if available)
        if RAG_AVAILABLE and knowledge_retriever:
            logger.info("Retrieving context using RAG...")
            rag_context = knowledge_retriever.retrieve_context(content)
        else:
            logger.info("RAG not available, using fallback context")
            rag_context = {
                "context": "Basic Tally support context",
                "chosen_link": "https://help.tallysolutions.com/tally-prime/",
                "raw_steps": ["Check Tally documentation", "Contact support"]
            }
...
async def test_rag_status():
    ...
    try:
        if not RAG_AVAILABLE or not knowledge_retriever:
            return {
                "status": "unavailable",
                "message": "RAG pipeline not available - using fallback responses",
                "components": {
...
async def full_analysis_endpoint(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Step 1: Transcribe audio (your existing transcription)
        try:
            transcription = transcribe_audio_file(file)
        except APIError as api_err:
