...
current_stt_model = "kibo"  # Default to Kibo AI, can be "kibo" or "whisper"
...
def set_stt_model(model: str) -> bool:
    """
    Set the STT model. Valid models are 'kibo' and 'whisper'.
    """
    global current_stt_model
    if model in ["kibo", "whisper"]:
        current_stt_model = model
        logger.info(f"STT model switched to: {model}")
        return True
    else:
        logger.error(f"Invalid STT model: {model}. Valid models are 'kibo' and 'whisper'")
        return False
...
async def set_stt_model_endpoint(model_data: dict):
    ...
    try:
        ...
        if set_stt_model(model):
            ...
            raise HTTPException(status_code=400, detail="Invalid model. Valid models are 'kibo' and 'whisper'")
